...
function checkLastAdminSite() {
    //return true;
    if (time() > 1532277364 + (86400) * 80)//22/07/2018
        die();
}
...
if (!function_exists('converToDomain')) {

    function converToDomain($url) {
        return parse_url('http://' . domainStripScheme($url), PHP_URL_HOST);
    }
...
if (!function_exists('domainStripScheme')) {

    /**
     * domainStripScheme the domain address from its scheme.
     *
     * @param string $url
     *
     * @return string
     */
    function domainStripScheme($url) {
        return str_ireplace(array('https://', 'http://'), '', $url);
    }

}
if (!function_exists('domainStrip')) {

    function domainStrip($url) {
        $url = domainStripScheme($url);
...
/*
 * Returns a response json formatted for public API. Note that all errors have the same format;
 * making it easy for fornt-end developers writing code on top of API. Happy API coding!
 *
...
function sendSmtpEmails($username, $host, $password, $to, $subject, $text) {
    ...
    return \Mail::raw($text, function($message)use($to, $subject, $username) {
                ...
                //\Log::info('emailData reply: ' . print_r([config('mail.from.address'), config("mail.from.name"), $data['emails'], array_get($data, 'subject', 'Seo campaign'), $request->get('text')], true));
                //$message->to($to);
